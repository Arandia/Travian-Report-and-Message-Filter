...
/*****************************************************************************
 * Copyright 2008, Adriaan Tichler
 *
 ...
 * Dec  23, 2008 | Arandia      | Started rework of vassals to add player name, not just village name
 * Dec  29, 2008 | Arandia      | Continued with rework
 * Jan  03, 2009 | Arandia      | Modified 'view messages' link to accomodate rework
 ****************************************************************************************************************************/
...
function vassal(){
    ...
    <table cellspacing="1" cellpadding="2">\
        <tr>\
            <td><input class="fm" type="text" maxlength="20" size="16" value="" id="newVassal"/></td> \
...
function profileOnClick(){
    name = xPathEval('//td[@class="rbg"]').snapshotItem(0).childNodes[0].childNodes[0].textContent.split(' ');
    name.splice(0, 1); // Remove the first element, 'Player'... could be language-dependant
    name = name.join(' ')
...
/*****************************************************
 * This handles vassalizing a player...
 *****************************************************/
function vassalizePlayerOnClick(){
...
function profile(){
    ...
    // We want to append the 'view messages' link just after the 'write message' link
    ...
    // Add the link to list the entire player in the vassal table
    h = xPathEval('//td[@class="rbg"]').snapshotItem(0);
    div = document.createElement('div');
    div.innerHTML = h.textContent + '&nbsp;<a id="vassalizePlayer" href="javascript:void(0)"><img src="'+
	image['+']+'" title="'+t['Add as vassal']+'"/></a>';
    h.textContent = '';
    h.appendChild(div);
    document.getElementById('vassalizePlayer').addEventListener('click', vassalizePlayerOnClick, false);

    // Add the link to list each village individually in the vassal table
    table = xPathEval('//table[@class="tbg"]').snapshotItem(1).childNodes[1];
    for (var i=3; i < table.childNodes.length; i++){
	a = document.createElement('a');
	a.innerHTML = '&nbsp;<img src="'+image['+']+'" title="'+t['Add as vassal']+'"/>';
	a.href = 'javascript:void(0)';

	// This is Beyond-insensitive... :D
	table.childNodes[i].childNodes[0].insertBefore(a, table.childNodes[i].childNodes[0].childNodes[1]);

	a.addEventListener('click', function(){dbg('hi');}, false);
    }
}
...
function loadImages(){
    ...
    image['+'] = 'data:image/bmp;base64,';
}